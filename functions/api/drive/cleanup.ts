...
export const onRequestPost: PagesFunction<Env> = async (context) => {
    try {
        ...
        if (dbCustomers.length === 0) {
            return Response.json({ success: true, deletedCount: 0, message: "No customers with drive mapping." });
        }
        ...
        do {
            ...
            data.files.forEach(f => allDriveFiles.add(f.id));
        ...
        return Response.json({ success: true, deletedCount: deletedIds.length, deletedIds });
    } catch (e) {
